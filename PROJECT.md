...
- **The same Active checkbox now sits on every Base/Flex Income and Base/Flex Spend row too**, not just Wishlist — same dimmed-row, instantly-reversible behavior, excluded from that category's total (and therefore Surplus, Daily Allowance, and Money Saved) while unchecked. The one difference from Wishlist: a freshly created item in these four categories defaults **Active on**, since they're the standing/planned figures the app is built around, not playground placeholders — defaulting them off would silently zero out a newly added rent or salary line until someone remembered to check it.
...
**The Active checkbox extended from Wishlist to all four Month Setup screens** (Base Income, Flex Income, Base Spend, Flex Spend) — see [Active / inactive](#active--inactive-replaces-the-old-purchased-mechanic) above. `categoryTotal()` in `formulas.ts` now filters on `active` for every category, not just Wishlist; `CategoryBars.tsx` grew the same row-level checkbox and dimmed-row styling `WishlistBody.tsx` already had, wired through `MonthSetupScreen.tsx` to the existing (already category-generic) `setActive` write in `DataContext.tsx`. New items in these four categories default **Active on** (Wishlist's own new items still default off) — `addItem` now sets `active: category !== 'wishlist'`, and existing items with no `active` field on record already read as active via `itemFromDoc`'s `raw.active !== false` fallback, so nothing already in Firestore changed behavior. Covered by an added Vitest case; 29 total.
