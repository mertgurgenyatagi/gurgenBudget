/**
 * Every day boundary in gurgenBudget is Turkey time, never the device's local time.
 * Turkey has been on a fixed UTC+3 with no daylight saving since 2016, so a plain
 * offset is correct — no timezone library needed.
 */
...
/** The current instant, shifted so that UTC getters read as Turkey wall-clock time. */
...
/** Day-of-month number, Turkey time. */
...
/** Weekday name for a day, Turkey time. */
...
/**
 * How many days of `month` have fully passed (are loggable).
 * Logging is inherently retrospective — today is never loggable, since the day
 * isn't over yet for the user.
 */
